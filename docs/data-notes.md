...
- **Semester coverage — which layers reach which semester (caveat Sem 3/4):** `delivered_niat` and `academic_plan_derived` carry rows for all 4 semesters, but **Sem-4 delivery is entirely the internal NIAT entity ("Nxtwave Institute of Advanced Technologies") — no partner college has Sem-4 data** — so `college_summary` (real colleges only) spans **Sem 1-3**. And `delivered_sessions` / `session_link` / **feedback** / **content** stop at **Sem 1+2** — the fuzzy bridge collapses to **~5% (Sem 3), 0% (Sem 4)** — so **Sem 3/4 have essentially no content and only trace feedback** (only the ~5% of Sem-3 sessions that fuzzy-link carry any; **`college_summary`'s Sem 3/4 rating columns are NULL** because it maps feedback via `delivered_sessions` = Sem 1+2, whereas the Knowledge Base uses the looser `session_link` and will show a few). Treat Sem 3/4 feedback/content as sparse-to-absent, not authoritative. Designed plans (`course_plan_vs_actual`, `deviation`, `designed_sequence`) are **Sem 1 only**; `subject_tags` is Sem 1+2 (Sem 3 sparse, Sem 4 none). For a Sem 3/4 question, answer from `delivered_niat` / `academic_plan_derived` and **say feedback/content/designed-plan aren't available for that semester** — don't imply zero. (A stray Jun–Jul 2026 tail of ~5k sessions can skew min/max date windows; prefer explicit semester filters.)
...
| `college_summary` | one row per college × **semester** | **At-a-glance health per college** — sections, courses, scheduled_sessions, pct_completed, teaching_weeks, first/last_session, avg_understanding, avg_teaching, recorded_issues, has_designed_plan. Use for "how is X doing", "compare colleges", "which college is struggling". Covers **Sem 1-3** (real colleges; **Sem-4 delivery is the internal NIAT entity only, excluded**) — filter `WHERE semester=…` for one. Feedback `avg_*` NULL for Sem 3 (feedback maps via `delivered_sessions` = Sem 1+2); `has_designed_plan` true only for Sem 1. |
