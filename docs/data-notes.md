...
- **There is no Subject entity.** `courses.stack` (11 stacks) is the closest roll-up. **`courses.stack` + `grit-skill-course-map.md` bridge GRIT skills → catalogue courses → delivery:** each GRIT skill maps to a stack; join `courses.course_ids` → `student_performance.course_id` (dash-less, ~60% resolve) to check GRIT-L1 coverage against real student mastery.
