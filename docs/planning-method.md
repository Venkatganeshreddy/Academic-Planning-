...
3. **Weekly hours must sum to the HLID total.** The sum of every week's "Courses running (hrs)" must equal the HLID's total course hours (session + practice + micro), within rounding. If the weekly cell shows only lecture hours, **say so** and still make each course's weekly total tie back to §4.
4. **Name the three assessment pools separately, never interchangeably:** **Skill Assessment** (30 h, *off* the 495 budget), **Module Quiz** (45 h, *off* budget), **Micro Assessment** (*inside* the 495 course hours). The 495 = 630 − 30 − 45 − 60 buffer has already removed the first two — don't double-count them.
