...
Every number in the plan then traces back to one of these — and carries its citation (above). Keep the through-line in view across all layers: the plan exists so the **student actually learns and finishes** (completion, not just coverage) — and GRIT skills/bands are the employability yardstick for any placement-facing call (use `grit-skill-course-map.md`'s **L1 coverage/gap list** as the curriculum-readiness check; the named Year-1 blocker is CS Fundamentals L1 OS/networking). A change that serves neither isn't an improvement. Be honest about gaps, and **ask** when a material input is missing.
...
**Reconcile BEFORE emitting — the sections must agree with each other, not just be individually grounded. A plan that fails any of these is not done:**
1. **Teaching weeks are counted, not claimed.** A teaching week is one with actual classes — **exclude break weeks and exam weeks**. The window must give **≥ 15 real teaching weeks** (AICTE floor); if the calendar has fewer, **extend the end date** until it does, and state the true count. Compute `hrs/week = total course hours ÷ real teaching weeks` — **never divide by calendar weeks**, and never count an exam/break week as instructional.
2. **Every HLID Start/End week must be a teaching week.** A course cannot run in a break or exam week. Its stated duration = the teaching weeks it actually appears in the weekly calendar; cross-check §4's Start/End against §6's grid — they **must match** (no course "starting" on the break week).
3. **Weekly hours must reconcile with the HLID — and say what they measure.** The weekly "Courses running (hrs)" is the **lecture + practice** teaching load; the **micro-assessment hours sit in the assessment slots**, not the weekly course line. So the weekly grid must sum to **(session hrs + practice hrs)** from §4 — NOT the grand total that also includes micro — within rounding. State this in one line under the calendar. **Never label the weekly hours "session + practice + micro" if micro isn't actually in them** (that was the old mismatch: weekly summed to ~341 while the plan claimed the 412 grand total).
4. **Name the three assessment pools separately, never interchangeably — and print them as a one-line glossary** right under the HLID totals so the reader can't conflate them: **Skill Assessment** (30 h, *off* the 495 budget), **Module Quiz** (45 h, *off* budget), **Micro Assessment** (*inside* the 495 course hours). The 495 = 630 − 30 − 45 − 60 buffer has already removed the first two — don't double-count them.
5. **Week boundaries follow the teaching days.** If the daily rhythm is Mon–Sat, bound each weekly row Mon–Sat — don't start a week on Sunday.
...
> **Content-readiness — check name variants before declaring "no content."** A course's content is often filed under **component or catalogue names, not the university's course label**, so an exact-name lookup in `content_all` under-reports. Before flagging a course as "no ingested content", also check its variants — e.g. **Web Application Development-1** → *Build Your Own Static / Responsive Website · Modern Responsive Web Design · JS Essentials · React*; **Mathematics for Computing** → *Mathematics for Computer Science*; **Computer Programming** → *Introduction to Software Development / Programming Foundations*. Only call it a gap if none of the variants have content.
...
- **Anchor "placement readiness" in the GRIT skills catalogue** (`grit-programme.md` — the copilot's second product). The skills GRIT rewards, and their salary bands, are a proxy for what employers hire for: where a change targets employability, name the specific GRIT **skill + level** it builds toward (and its band). **GRIT L1 is the Year-1 readiness benchmark** — use [`grit-skill-course-map.md`](grit-skill-course-map.md) to check which delivered Year-1 courses cover each GRIT skill/level, and flag the gaps. (Per that map the current Year-1 **gaps** are System Design, DS & ML, Data Intelligence, Physical AI, and CS Fundamentals L1's OS/networking — whereas UI/Server-Side Engineering, SQL, Quant Reasoning, Computational Thinking, Gen AI and Critical Thinking L1 are already covered.)
