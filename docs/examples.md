...
1. `## Inputs & grounding` — subject→course map (Computer Programming → S-VYASA's "Problem Solving Using Programming", etc., from `subject_tags`); grounded in S-VYASA's own delivery (`academic_plan_derived` / `course_plan_vs_actual`); window: Jan 6–Apr 25 ≈ 16 weeks, 1 named break → 15 instructional; 495h budget. **GRIT L1 check** (`grit-skill-course-map.md`): these three subjects build Computational Thinking / Quantitative Reasoning / Critical Thinking L1 (covered) — this subject set doesn't reach CS Fundamentals L1 (OS/networking) or the other gap skills, so flag that if placement-readiness is the goal.
